...
def sync_spotify_music():
    try:
        ...
        if results['items']:
            ...
            spotify_album_id = latest_album['id']  # ← ADDED
            ...
            # ← ADDED: save Spotify ID to settings for home page embed
            if not settings:
                settings = Settings(artist_id=artist_id, latest_release_id=spotify_album_id)
                db.session.add(settings)
            else:
                settings.latest_release_id = spotify_album_id
            db.session.commit()
